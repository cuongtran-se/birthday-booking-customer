...
const PackageCard = ({
  ...
  pkg: PackageDataResponse;
  bookingData: BookingRequest | null;
  setItem: (id: PackageDataResponse) => void;
}) => {
  const {
    id,
    active,
  ...
  return packageType === SERVICE_ENUM.DECORATION ? (
    <Col span={6}>
      <Card
        hoverable
        style={{
          bottom: id === bookingData?.packageDecoId ? 10 : 0,
          boxShadow:
            id === bookingData?.packageDecoId
      ...
      >
        ...
        <Flex gap={10} align="center" className="mt-3">
          ...
          <Button type="primary" onClick={() => setItem(pkg)}>
            {id === bookingData?.packageDecoId ? "Đang chọn" : "Chọn"}
          </Button>
        </Flex>
      </Card>
    </Col>
    ...
    <Col span={6}>
      <Card
        hoverable
        style={{
          bottom: id === bookingData?.packageFoodId ? 10 : 0,
          boxShadow:
            id === bookingData?.packageFoodId
              ? "0 4px 8px rgba(153, 102, 255, 0.9)"
              : "unset",
        }}
        cover={
      ...
      >
        ...
        <Flex gap={10} align="center" className="mt-3">
          ...
          <Button type="primary" onClick={() => setItem(pkg)}>
            {id === bookingData?.packageFoodId ? "Đang chọn" : "Chọn"}
          </Button>
        </Flex>
      </Card>
    </Col>
